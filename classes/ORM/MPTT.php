...
class ORM_MPTT extends ORM
{
	...
	{
		$this->_db->query(Database::SELECT, 'SELECT RELEASE_LOCK("' . $this->_db->table_prefix() . $this->_table_name . '") AS l', TRUE);
	...
	private function create_space($start, $size = 2)
	{
		...
		$this->_db->query(Database::UPDATE, 'UPDATE '.$this->_db->table_prefix().$this->_table_name.' SET `'.$this->right_column.'` = `'.$this->right_column.'` + '.$size.' WHERE `'.$this->right_column.'` >= '.$start.' AND `'.$this->scope_column.'` = '.$this->{$this->scope_column}, FALSE);
		$this->_db->query(Database::UPDATE, 'UPDATE '.$this->_db->table_prefix().$this->_table_name.' SET `'.$this->left_column.'` = `'.$this->left_column.'` + '.$size.' WHERE `'.$this->left_column.'` >= '.$start.' AND `'.$this->scope_column.'` = '.$this->{$this->scope_column}, FALSE);
	}
	...
	protected function move($target, $left_column, $left_offset, $level_offset, $allow_root_target)
	{
		...
		// Update the values.
		$this->_db->query(Database::UPDATE, 'UPDATE '.$this->_db->table_prefix().$this->_table_name.' SET `'.$this->left_column.'` = `'.$this->left_column.'` + '.$offset.', `'.$this->right_column.'` = `'.$this->right_column.'` + '.$offset.'
	...
	private function get_scopes()
	{
		...
		// used by verify_tree()
		return $this->_db->query(Database::SELECT, 'SELECT DISTINCT(`'.$this->scope_column.'`) from `'.$this->_db->table_prefix().$this->_table_name.'`', TRUE);
	}
	...
	public function verify_scope($scope)
	{
		...
		$result = $this->_db->query(Database::SELECT, 'SELECT count(*) as count FROM `'.$this->_db->table_prefix().$this->_table_name.'` WHERE `'.$this->scope_column.'` = '.$root->{$this->scope_column}.' AND (`'.$this->left_column.'` > '.$end.' OR `'.$this->right_column.'` > '.$end.')', FALSE);
		if ($result[0]->count > 0)
			return FALSE;

		// Find nodes that have the same left and right value
		$result = $this->_db->query(Database::SELECT, 'SELECT count(*) as count FROM `'.$this->_db->table_prefix().$this->_table_name.'` WHERE `'.$this->scope_column.'` = '.$root->{$this->scope_column}.' AND `'.$this->left_column.'` = `'.$this->right_column.'`', FALSE);
		if ($result[0]->count > 0)
			return FALSE;
		...
		while ($i <= $end)
		{
			$result = $this->_db->query(Database::SELECT, 'SELECT count(*) as count FROM `'.$this->_db->table_prefix().$this->_table_name.'` WHERE `'.$this->scope_column.'` = '.$root->{$this->scope_column}.' AND (`'.$this->left_column.'` = '.$i.' OR `'.$this->right_column.'` = '.$i.')', FALSE);
